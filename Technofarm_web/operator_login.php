...
if(isset($_POST['login']))
	...
	$category ="Field Operator"; //Only Field Operators can login here
	if(empty($id) || empty($password)){
		$error.= "Please fill in all the details before you proceed";
	}else{
		$sql = "SELECT * FROM users WHERE idno='$id' and password='$password' and category ='$category' ";//compares if id,password and category are correct
		$query = mysql_query($sql);
		@ $result = mysql_fetch_array($query);
		if($result > 0){
			$_SESSION['id'] = $id;
			header("Location:../Technofarm/operator/plant.php");
		}else{
...
<div id="cont">
	...
	<div id="content">
            ...
            <a href="../Technofarm/logout.php"><input type="button" value="Back" size="30"/></a>
			</td>
			</tr>
			</table>
			</form>

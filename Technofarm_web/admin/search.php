...
<a class="addnew" href="view_users.php" style="font-face:Khmer OS Battambang; font-size:16px;">Back</a></font>
    ...
    <?php
		if($cbo=="ID Number")
		{
			$id = mysql_query("SELECT * FROM users WHERE idno like '".$search."%'");
	?>
	...
	?>
		...
		{
			$na = mysql_query("SELECT * FROM users WHERE fname like '".$search."%'");
	?>
    <?php
	...
	?>
     ...
     <?php
				...
				{
        $add = mysql_query("SELECT * FROM users WHERE category like '".$search."%'");
     ?>
		<?php
		...
		?>
			...
			{
			$g = mysql_query("SELECT * FROM users WHERE county like '".$search."%'");
			?>  
			<?php
				while($co=mysql_fetch_array($g))
				{			
			?>
            <tr>
				<td><?php echo $co[0]; ?></td>
